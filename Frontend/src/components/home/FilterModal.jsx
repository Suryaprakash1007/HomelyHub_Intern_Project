...
const FilterModal = ({ selectedFilters, onFilterChange, onClose }) => {
  const [priceRange, setPriceRange] = useState({
    min: selectedFilters.minPrice || MIN_PRICE,
    max: selectedFilters.maxPrice || MAX_PRICE,
  });
  ...
  const handleMinInputChange = (e) => {
    ...
    setPriceRange((prev) => ({ ...prev, min: isNaN(minValue) ? MIN_PRICE : minValue }));
  };

  const handleMaxInputChange = (e) => {
    const maxValue = parseInt(e.target.value, 10);
    setPriceRange((prev) => ({ ...prev, max: isNaN(maxValue) ? MAX_PRICE : maxValue }));
  };
  ...
  const clampedValue = {
    min: Math.max(MIN_PRICE, Math.min(priceRange.min || MIN_PRICE, MAX_PRICE)),
    max: Math.min(MAX_PRICE, Math.max(priceRange.max || MAX_PRICE, MIN_PRICE)),
  };
