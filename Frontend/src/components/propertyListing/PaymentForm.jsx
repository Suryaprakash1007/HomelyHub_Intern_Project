...
const PaymentForm = ({
  ...
  const bookingsList = Array.isArray(currentBookings) ? currentBookings : [];
  ...
  const isDateDisabled = (current) => {
    ...
    return bookingsList.some((booking) => {
      if (!booking || !booking.fromDate || !booking.toDate) return false;
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      if (name && guests && phoneNumber && checkinDate && checkoutDate) {
        ...
        navigate(`/payment/${propertyId}`, { state: { paymentDetails } });
      } else {
